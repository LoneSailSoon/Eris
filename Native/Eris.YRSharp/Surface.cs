...
public struct Surface
{
        ...
        int ZS_Y)
    {
        var func =
            (delegate* unmanaged[Thiscall]<nint, nint, nint, nint, int, nint, nint, BlitterFlags, uint,
                int, uint, uint, int, nint, uint, int, int, void>)NativeCode.FastCallTransferStation;
    ...
    public static unsafe bool GetTextDimension(string text, out int width, out int height, int nMaxWidth)
    {
        width = 0;
        height = 0;
        using var span = new UniStringSpan(text);
        return ((delegate* unmanaged[Thiscall]<nint, nint, nint, nint, int, bool>)0x433CF0)(BitFont, span, width.GetThisPointer(), height.GetThisPointer(), nMaxWidth);
    ...
    public unsafe void BitTextDraw(string text, RectangleStruct rect)
    {
        ...
        func(BitText, BitFont, this.GetThisPointer(), span, rect.X, rect.Y, rect.Width, rect.Height, 0, 0, 0);
